...
st.set_page_config(page_title='OTUS.ML.ADV_HW2', page_icon='../backend/models/car_at_night.ico',
...
st.image('../backend/models/sky.png')
...
with st.expander("Show code", expanded=False):
    ...
    st.set_page_config(page_title='OTUS.ML.ADV_HW2', page_icon='../images/car_at_night.ico',
                    layout='centered', initial_sidebar_state='expanded')
    ...
    # Main page button
    if send_req:
        prediction = requests.get("http://192.168.1.64:8000/predict/", 
                                    params={"q": tuple(x17.values)})
        st.code(f'Parameters sent: {x17.values}')
        col1, col2 = st.columns(2)
        with col1:
...
# atom = ATOMLoader("../notebooks/atom", verbose=0)
atom = ATOMLoader("../data/atom20220104", verbose=0)
...
if __name__ == '__main__':
    uvicorn.run("app:app", host="192.168.1.64", port=8000, reload=True)        
        """
)
